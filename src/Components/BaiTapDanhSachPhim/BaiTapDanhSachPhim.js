...
let renderDanhSachPhim = () => {
    ...
    return dataPhim.map((phim, index) => {
        return <div className="col-4" key={index}>
            <div className="card">
                <img className={`card-img-top ${style.hinhAnh__Phim}`} src={phim.hinhAnh} alt />
                <div className="card-body">
                    <h4 className="card-title text-primary" style={{ height: '50px' }}> {phim.tenPhim.length > 50 ? <span>{this.tenPhim.substr(0, 50)}...</span> : <span>{phim.tenPhim}</span>}</h4>
                    <p className="card-text">{phim.moTa.length > 150 ? <span>{phim.moTa.substr(0, 150)}...</span> : <span>{phim.moTa}</span>}</p>
                </div>
            </div>
        </div>
...
export default class BaiTapDanhSachPhim extends Component {

    // ở đây là khai báo thuộc tính
    ...
    render() {
        return (
            <div className="container">
                <h2 className='text-danger text-center'>Danh Sach Phim</h2>
                <div className="row">
                    {renderDanhSachPhim()}
                </div>
            </div>
